...
def get_main_keyboard(user_id: int):
    ...
    buttons = [
        ...
        [
            InlineKeyboardButton(text="📊 View status / Buy Slots ⭐", callback_data="view_quota")
        ]
    ]
    if user_id in admin_ids:
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    if is_user_banned(user_id):
        await message.answer("🚫 You are banned from this OWNER.")
        return
    
    await update_user_activity(user_id)

    welcome_text = """
👋 Welcome to the keshvexff OG 24/7 🥶 HOSTER !
...
async def cmd_new_project(message: types.Message, state: FSMContext):
    ...
    if len(projects) >= slot_limit:
        await message.answer(
            ...
            "Please delete an existing project or buy a new slot from the 'My status' menu.".format(slot_limit)
        )
...
async def callback_my_projects(callback_or_message: Union[types.CallbackQuery, types.Message]):
    ...
    if not projects:
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="📊 View My status", callback_data="view_quota")]
        ])
    else:
        ...
        buttons.append([InlineKeyboardButton(text="📊 View My status", callback_data="view_quota")])
...
""".format(project_name, status_icon, status.capitalize(), '9999MB (Free)' if ram == 'free' else '9999MB (Premium)', run_cmd)
...
async def web_server():
    ...
    async def handle(request):
        return web.Response(text="🚀 Python keshvexff Hoster Bot is running!")
...
async def main():
    logger.info("🚀 Starting keshvexff Hoster Bot...")
    await init_db()
    await migrate_db()
